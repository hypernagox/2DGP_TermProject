...
class CCamera(CComponent):
    ...
    def world_to_screen(self,world_pos):
        ...
        screen_x = world_pos.x - self.m_transform.x + width / 2
        screen_y = world_pos.y - self.m_transform.y + height / 2
        return Vec2(screen_x, screen_y)
    def screen_to_world(self,screen_pos):
        width, height = CCore().GetWidthHeight()
        screen_x = screen_pos.x - self.m_transform.x + width / 2
        screen_y = screen_pos.y - self.m_transform.y + height / 2
        return Vec2(screen_x, screen_y)
